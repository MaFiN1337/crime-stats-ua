...
![Figure 1. 111 vs 111-1 radar](/info/111/fig_111_radar_articles.png)
...
![Figure 2. MICE–RASCLS map](/info/111/fig_111_mice_rascls_map.png)
...
![Figure 3. Flow from factors to legal articles](/info/111/fig_111_flow_factors_mice_rascls.png)
...
### 6.1. Factor frequency profile

Figure 4 shows the sum of each factor across all cases. With a single case, this is equivalent to that case’s 0/1 profile.

![Figure 4. Frequency of factors in the pilot dataset](/info/111/fig_111_bar_factor_frequency.png)

Interpretation:

- Present (1): Money, Ideology, Ego, Reciprocation, Authority, Commitment, SocialProof.
- Absent (0): Coercion, Scarcity, Liking.

The profile suggests a combination of financial and ideological motives, supported by authority, reciprocity and social proof, with no visible role for coercion or economic scarcity.

### 6.2. MICE profile by actor type

Currently the dataset contains a single actor type: “Member of Parliament / media figure”.

![Figure 5. MICE profile by actor type](/info/111/fig_111_bar_mice_by_type.png)

Interpretation:

- High Money, Ideology and Ego; zero Coercion.
- This matches the idea of a powerful actor who:
  - shares or is comfortable with the aggressor’s narratives;
  - benefits financially;
  - leverages their status and media presence.

When further actor types are added (local officials, business on TOT, ordinary residents), this figure will allow comparisons between groups.

### 6.3. Factor profiles by article

Figure 6 is a radar chart of average factor values per legal article.

![Figure 6. Factor profiles by article (111 vs 111-1 – pilot)](/info/111/fig_111_radar_profiles_by_article.png)

At the moment:

- only Article 111 is represented (one case), so we see a single profile.
- an average profile for 111;
- an average profile for 111-1,

## 7. Conclusion

This module demonstrates how legal norms, motivational models and simple data visualisations can be combined to analyse high treason in Ukraine.

Key points:

- Article 111 CCU targets deliberate, high-impact acts by Ukrainian citizens that directly harm state security.
- Article 111-1 covers a broader range of collaboration acts, often on TOT, with heterogeneous levels of danger.

The MICE model helps structure why individuals cooperate with the aggressor:
- financial interests (Money),
- ideological alignment (Ideology),
- coercion (Coercion/Compromise),
- personal ambition and status (Ego).

The RASCLS model explains how these motives are activated:
- obligations and transactions (Reciprocation),
- use of formal and informal authority,
- exploitation of scarcity,
- pressure to remain consistent, liking and social proof.

A pilot case-based dataset for the Dubinsky high-treason suspicion illustrates how real-world cases can be manually coded into these factors. Even with one case, the charts highlight a coherent profile dominated by Money, Ideology, Ego, Authority and SocialProof.
The approach is extensible: by adding more coded cases under Articles 111 and 111-1, analysts can explore:
- typical motivational profiles for different actor types;
- systematic differences between high treason and collaboration;
- temporal and regional patterns.

At the same time, the analysis is constrained by:
- the small size of the dataset;
- reliance on open-source information;
- subjectivity of manual coding.
